...
@testset "method-recovery gate · U1SU2 engine == U1U1 spectrum (2S+1)-expanded" begin
    ...
    for nsites in (2, 3)
        alg_su2 = NRGAlgorithm(;
            discretization=disc, symmetry=U1SU2(), truncation=keepall, nsites
        )
        alg_u1 = NRGAlgorithm(;
        ...
        esu2 = sort([
            round(e; digits=6) for (e, twoS) in rsu2.levels[end] for _ in 1:(twoS + 1)
        ])
    ...
    r = nrg_solve(
        m,
        NRGAlgorithm(; discretization=disc, symmetry=U1SU2(), truncation=keepall, nsites=3),
    )
    ...
    r0s = nrg_solve(
        m0,
        NRGAlgorithm(; discretization=disc, symmetry=U1SU2(), truncation=keepall, nsites=2),
    )
    r0u = nrg_solve(
        m0,
    ...
    @test sort([
        round(e; digits=6) for (e, twoS) in rs.levels[end] for _ in 1:(twoS + 1)
    ...
    rfp = nrg_solve(
        m,
        NRGAlgorithm(;
            discretization=disc, symmetry=U1SU2(), truncation=KeepN(120), nsites=40
        ),
    )
